...
lgn_de6 <- function(n){
  ech <- sample(x = 1:6, size = n, replace = TRUE)      # on lance un dé à 6 faces n fois
  res <- data.frame()
  for (i in 1:n) {
  ...
  colnames(res) <- c("x", "p")
  plot(res$x, res$p, 
       main = "Moyenne des lancers de dé",
...
tcl <- function(nb_sim, taille_ech){
  ...
  hist(res, 
       ...
       main = "Distribution des moyennes\n des échantillons simulés selon une loi uniforme")
  lines(density(res), col = 4, lwd = 2)
}

# Pour 1000 simulations d'un échantillon de taille 50
tcl(1000, 50)